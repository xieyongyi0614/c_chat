...
- [x] 确认 `LocalMessageListItem` 字段。
- [x] 确认 `MessageStatus` 状态含义。
- [x] 确认文本消息发送入参和返回。
- [x] 确认媒体消息由上传 complete 后服务端建消息。
- [x] 确认历史分页参数和返回结构。
- [x] 确认已读接口返回 `lastReadSeq` 和 `unreadCount`。
...
- [x] 新增或确认本地历史读取方法。
- [x] 新增或确认已读同步方法。
- [x] 新增推送订阅封装，订阅 `ackSendMessage`、`newUpdateMessage`、`ReadMessageResponse`。
...
- [x] 建立 message 对象仓库。
- [x] 设计按 `id`、`clientMsgId`、`conversationId` 的索引。
- [x] 实现消息 upsert。
- [x] 实现 pending 消息写入。
- [x] 实现服务端消息覆盖 pending。
- [x] 实现历史分页本地读取。
