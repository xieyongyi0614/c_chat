...
- [ ] 确认 `shared-api` 替代 IPC + 主进程网络层。
- [ ] 确认 `shared-protobuf` 原样复用。
- [ ] 确认 IndexedDB 替代 SQLite。
- [ ] 确认 Lightbox 替代独立预览窗口。
- [ ] 确认 File API 替代本地路径。
- [ ] 确认 `audio-core` + 浏览器 API 替代 Electron 音频。
- [ ] 确认托盘、自定义窗口控制、多窗口多账号不进入一期。
...
- [ ] 认证模块引用单账号取舍。
- [ ] 传输模块引用 `shared-api` 和 IndexedDB 取舍。
- [ ] 会话与消息模块引用 IndexedDB 取舍。
- [ ] 上传与预览模块引用 File API 取舍。
- [ ] 语音模块引用 `audio-core` 和浏览器音频取舍。
- [ ] 群聊模块引用共享会话和消息底座。
...
- [ ] 检查是否存在自定义窗口控制任务。
- [ ] 检查是否存在多窗口多账号任务。
- [ ] 检查是否存在 Web 专用协议或网络 client 任务。
- [ ] 检查是否存在手写 shadcn 基础交互任务。
