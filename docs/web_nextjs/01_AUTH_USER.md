...
- [ ] 确认 `signIn`、`signUp`、`getUserInfo`、`updateUserProfile` 满足 Web 一期。
- [ ] 确认是否需要在 `shared-api` 补充 `logout`。
- [ ] 确认是否需要在 `shared-api` 补充 `getUserList`。
- [ ] 确认 token 持久化位置和 `TokenProvider` 行为。
- [ ] 确认登录成功后如何初始化 `RealtimeClient`。
...
- [ ] 实现登录页表单。
- [ ] 实现注册页表单。
- [ ] 接入 `AuthService.signIn`。
- [ ] 接入 `AuthService.signUp`。
- [ ] 登录/注册成功后保存 token。
- [ ] 登录/注册成功后拉取当前用户信息。
- [ ] 登录/注册失败时展示错误反馈。
