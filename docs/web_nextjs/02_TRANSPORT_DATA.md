...
- [ ] 确认 Web 端 HTTPS 请求统一走 `packages/shared-api`。
- [ ] 确认 WebSocket 请求和推送统一走 `RealtimeClient`。
- [ ] 确认协议新增统一走 `packages/shared-protobuf`。
- [ ] 确认页面层不直接拼 socket event 和 protobuf payload。
- [ ] 确认缺失业务 service 时扩展 `shared-api`。
...
- [ ] 实现 Web 端 `TokenProvider`。
- [ ] 实现 Web 端 `IdentityProvider`。
- [ ] 实现 Web 端 `ClientInfo`。
- [ ] 实现 Web 端 `ErrorReporter`。
- [ ] 实现 Web 端 `ConnectionObserver`。
- [ ] 用 `createApiClient` 创建 Web API client 单例。
...
- [ ] 使用 Web adapter 创建 `RealtimeClient`。
- [ ] 登录后建立 socket 连接。
- [ ] 验证 `ping -> pong` 请求配对。
- [ ] 注册 `newUpdateMessage`、`newConversation` 等服务端推送监听。
- [ ] 登出时销毁连接并清理挂起请求。
...
- [ ] 核对认证 service 是否满足 Web 登录、注册、资料编辑。
- [ ] 核对上传 service 是否满足 File API 分片上传。
- [ ] 补齐会话 service：会话列表、本地会话、创建会话、已读。
- [ ] 补齐消息 service：发送、重发、历史、推送订阅。
- [ ] 补齐群聊 service：创建群、详情、更新、邀请、退出、解散。
